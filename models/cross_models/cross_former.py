...
class Crossformer(nn.Module):
    def __init__(self, data_dim, in_len, out_len, seg_len, win_size = 4,
                ...
                dropout=0.0, baseline = False, device=torch.device('cuda:0'), flag=None, plugin_args=None):
        ...
        if self.flag == 'Plugin' and plugin_args is not None:
            self.plugin = Plugin(plugin_args, data_dim)
    ...
    def forward(self, x_seq, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        x_enc = x_seq.clone()
        if (self.baseline):
        ...
        pred = base + predict_y[:, :self.out_len, :]
        if self.plugin is not None and x_mark_enc is not None and x_mark_dec is not None:
            pred = self.plugin(x_enc.clone(), x_mark_enc.clone(), pred, x_mark_dec.clone()[:, -self.out_len:, :])
