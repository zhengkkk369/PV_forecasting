## GLAFF-D3A Fusion batch launcher
online_learning='full'
i=1
ns=(1 )
bszs=(1 )
lens=(48)
glaff_backbone=('dlinear' 'patchtst' 'fedformer' 'autoformer')
methods=('glaff_d3a_fusion')
sleep_interval=16
sleep_epochs=20
for n in ${ns[*]}; do
for bsz in ${bszs[*]}; do
for len in ${lens[*]}; do
for m in ${methods[*]}; do
for bb in ${glaff_backbone[*]}; do
online_adjust=0.5
offline_adjust=0.5
CUDA_VISIBLE_DEVICES=0 nohup python -u main.py --method $m --glaff_backbone $glaff_backbone --root_path ./data/ --n_inner $n --test_bsz $bsz --data ETTh2 --features M --seq_len 60 --label_len 0 --pred_len $len --des 'Exp' --itr $i --train_epochs 10 --learning_rate 1e-3 --online_learning $online_learning  --sleep_interval $sleep_interval --sleep_epochs $sleep_epochs --online_adjust $online_adjust --offline_adjust $offline_adjust > $bb-$m-ETTh2-d3afusion$len$online_learning-sleep$sleep_interval-epoch$sleep_epochs'_'online_adjust$online_adjust'_'offline_adjust$offline_adjust.out 2>&1 &
